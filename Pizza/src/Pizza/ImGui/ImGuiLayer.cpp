...
namespace Pizza {
	...
	void ImGuiLayer::OnUpdate()
	{
		...
		// 获取我们的Application，为了拿到后面的窗口大小
		Application& app = Application::Get();
		io.DisplaySize = ImVec2(app.GetWindow().GetWidth(), app.GetWindow().GetHeight());

		// 这里就完全是临时的Hack，因为此时还没有做Time系统和DeltaTime步进，
		// 所以强行Quick & Dirty的给做了一个DeltaTime
		float time = (float)glfwGetTime();
		io.DeltaTime = m_Time > 0.0f ? (time - m_Time) : (1.0f / 60.0f);
		m_Time = time;

		// 这里开始，标记了Render的内容，NewFrame提供了需要Render前的一切基本前置需求，
		// 在此之后可以任意Submit指令，直到下一个Render会将其Render出来。
		...
		// 将这个Demo Window用OpenGL的实现方式渲染出来
		ImGui::Render();
