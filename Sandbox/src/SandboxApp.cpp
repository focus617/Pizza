...
public:
		...
		: Layer("Example"), m_Camera(-1.6f, 1.6f, -0.9f, 0.9f), m_CameraPosition(0.0f)
	{
		...
		m_Shader.reset(new Pizza::Shader(vertexSrc, fragmentSrc));
		...
		m_BlueShader.reset(new Pizza::Shader(blueShaderVertexSrc, blueShaderFragmentSrc));
	}
    ...
    void OnUpdate(Pizza::Timestep ts) override
    {
		...
		for (int y = 0; y < 20; y++)
		{
			for (int x = 0; x < 20; x++)
			{
				...
				Pizza::Renderer::Submit(m_BlueShader, m_SquareVA, transform);
			}
		}
...
private:
    ...
    std::shared_ptr<Pizza::Shader> m_BlueShader;
